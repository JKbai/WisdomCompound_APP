...
static NSString *const NetworkResponseCache = @"XBNetworkResponseCache";
        ...
        [[AFNetworkReachabilityManager sharedManager] setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
            switch (status) {
                case AFNetworkReachabilityStatusUnknown:
                    networkStatus ? networkStatus(XBNetworkStatusUnknown) : nil;
                    break;
                case AFNetworkReachabilityStatusNotReachable:
                    networkStatus ? networkStatus(XBNetworkStatusNotReachable) : nil;
                    break;
                case AFNetworkReachabilityStatusReachableViaWWAN:
                    networkStatus ? networkStatus(XBNetworkStatusReachableWWAN) : nil;
                    break;
                case AFNetworkReachabilityStatusReachableViaWiFi:
                    networkStatus ? networkStatus(XBNetworkStatusReachableWiFi) : nil;
                    break;
                default:
                    break;
            }
...
#pragma mark -- 缓存描述文字
+ (NSString *)cachePolicyStr:(XBCachePolicy)cachePolicy
{
    switch (cachePolicy) {
        case XBCachePolicyIgnoreCache:
            return @"只从网络获取数据，且数据不会缓存在本地";
            break;
        case XBCachePolicyCacheOnly:
            return @"只从缓存读数据，如果缓存没有数据，返回一个空";
            break;
        case XBCachePolicyNetworkOnly:
            return @"先从网络获取数据，同时会在本地缓存数据";
            break;
        case XBCachePolicyCacheElseNetwork:
            return @"先从缓存读取数据，如果没有再从网络获取";
            break;
        case XBCachePolicyNetworkElseCache:
            return @"先从网络获取数据，如果没有再从缓存读取数据";
            break;
        case XBCachePolicyCacheThenNetwork:
...
{
    [self HTTPWithMethod:XBRequestMethodGET url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)POSTWithURL:(NSString *)url
        ...
        cachePolicy:(XBCachePolicy)cachePolicy
            callback:(HttpRequest)callback
{
    [self HTTPWithMethod:XBRequestMethodPOST url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)HEADWithURL:(NSString *)url
        ...
        cachePolicy:(XBCachePolicy)cachePolicy
            callback:(HttpRequest)callback
{
    [self HTTPWithMethod:XBRequestMethodHEAD url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)PUTWithURL:(NSString *)url
       ...
       cachePolicy:(XBCachePolicy)cachePolicy
           callback:(HttpRequest)callback
{
    [self HTTPWithMethod:XBRequestMethodPUT url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)PATCHWithURL:(NSString *)url
         ...
         cachePolicy:(XBCachePolicy)cachePolicy
             callback:(HttpRequest)callback
{
    [self HTTPWithMethod:XBRequestMethodPATCH url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)DELETEWithURL:(NSString *)url
          ...
          cachePolicy:(XBCachePolicy)cachePolicy
              callback:(HttpRequest)callback
{
    [self HTTPWithMethod:XBRequestMethodDELETE url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
                ...
                callback:(HttpRequest)callback
{
    ...
    if (_logEnabled) {
        XBLog(@"\n请求参数 = %@\n请求URL = %@",parameters ? [self jsonToString:parameters]:@"空", url);
    }
    
    if (cachePolicy == XBCachePolicyIgnoreCache) {
        //只从网络获取数据，且数据不会缓存在本地
        [self httpWithMethod:method url:url parameters:parameters callback:callback];
    }else if (cachePolicy == XBCachePolicyCacheOnly){
    ...
    }else if (cachePolicy == XBCachePolicyNetworkOnly){
        //先从网络获取数据，同时会在本地缓存数据
    ...
    }else if (cachePolicy == XBCachePolicyCacheElseNetwork){
        //先从缓存读取数据，如果没有再从网络获取
        [self httpCacheForURL:url parameters:parameters withBlock:^(id<NSCoding> object) {
            if (object) {
                callback ? callback(object, nil) : nil;
    ...
    }else if (cachePolicy == XBCachePolicyNetworkElseCache){
    ...
    }else if (cachePolicy == XBCachePolicyCacheThenNetwork){
...
#pragma mark -- 网络请求处理
+(void)httpWithMethod:(XBRequestMethod)method url:(NSString *)url parameters:(NSDictionary *)parameters callback:(HttpRequest)callback{
    ...
    [self dataTaskWithHTTPMethod:method url:url parameters:parameters callback:^(NSURLSessionDataTask * _Nonnull task, id _Nullable responseObject) {
        if (_logEnabled) {
            XBLog(@"请求结果 = %@",[self jsonToString:responseObject]);
            
                if ([NSString isObject:responseObject[@"msg"]]) {
                    [HUDHelper showToastWithMessage:responseObject[@"msg"]];
                }
        ...
        if (_logEnabled) {
            XBLog(@"错误内容 = %@",error);
            [HUDHelper showToastWithMessage:@"服务器异常"];
        }
...
#pragma mark -- 上传文件
+ (void)uploadFileWithURL:(NSString *)url parameters:(NSDictionary *)parameters name:(NSString *)name filePath:(NSString *)filePath progress:(HttpProgress)progress callback:(HttpRequest)callback
{
...
#pragma mark -- 上传图片文件
+ (void)uploadImageURL:(NSString *)url parameters:(NSDictionary *)parameters images:(NSArray<UIImage *> *)images name:(NSString *)name fileName:(NSString *)fileName mimeType:(NSString *)mimeType progress:(HttpProgress)progress callback:(HttpRequest)callback;
{
    NSURLSessionTask *sessionTask = [_sessionManager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        //压缩-添加-上传图片
...
#pragma mark -- 下载文件
+(void)downloadWithURL:(NSString *)url fileDir:(NSString *)fileDir progress:(HttpProgress)progress callback:(HttpDownload)callback
{
    NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:url]];
    __block NSURLSessionDownloadTask *downloadTask = [_sessionManager downloadTaskWithRequest:request progress:^(NSProgress * _Nonnull downloadProgress) {
        if (_logEnabled) {
            XBLog(@"下载进度:%.2f%%",100.0*downloadProgress.completedUnitCount / downloadProgress.totalUnitCount);
        }
...
+ (NSString *)getMethodStr:(XBRequestMethod)method{
    switch (method) {
        case XBRequestMethodGET:
            return @"GET";
            break;
        case XBRequestMethodPOST:
            return @"POST";
            break;
        case XBRequestMethodHEAD:
            return @"HEAD";
            break;
        case XBRequestMethodPUT:
            return @"PUT";
            break;
        case XBRequestMethodPATCH:
            return @"PATCH";
            break;
        case XBRequestMethodDELETE:
            return @"DELETE";
            break;
...
+ (void)httpCacheForURL:(NSString *)url parameters:(NSDictionary *)parameters withBlock:(void(^)(id responseObject))block
{
    ...
    [_dataCache objectForKey:cacheKey withBlock:^(NSString * _Nonnull key, id<NSCoding>  _Nonnull object) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (_logEnabled) {
                XBLog(@"缓存结果 = %@",[self jsonToString:object]);
            }
            block(object);
        });
    }];
...
+ (void)setRequestSerializer:(XBRequestSerializer)requestSerializer{
    _sessionManager.requestSerializer = requestSerializer==XBRequestSerializerHTTP ? [AFHTTPRequestSerializer serializer] : [AFJSONRequestSerializer serializer];
}

+ (void)setResponseSerializer:(XBResponseSerializer)responseSerializer{
    _sessionManager.responseSerializer = responseSerializer==XBResponseSerializerHTTP ? [AFHTTPResponseSerializer serializer] : [AFJSONResponseSerializer serializer];
}
